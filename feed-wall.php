            ...
            <div class="main-content-inn col-three main-content-full usr-feed-page usr-feed-page-nw">
                <div class="col-first-side">
                    ...
                    <div class="pro-right-sec">
                        ...
                        <div class="sidebar">
                            <ul>
                                <li><a href="create-page.php"><span class="icn"><img src="images/create-page-icon.png" alt="icon" /></span>Create Page</a></li>
                    ...
                    <div class="col-first" id="load-post-ref">
                        ...
                        <div id="post-display">
                            <div class="container" id="posts-container">
                                ...
                                <?php include 'get_postdata.php'; ?></div>
                        </div>
...
var cur=$(this).parent().find('div#media.selected img').attr('src');
var nxt=$(this).parent().find('div#media.selected').next('div#media').find('img').attr('src');

    //$('.img-wrap div#media img.selected').hide().next().show().end().appendTo('.img-wrap');
	if($('.img-wrap div#media.selected').next().is('div#media')) {
var $ilink=$("<a>", {href: "foo", data-featherlight: "ajax"});
$ilink.click();
//var imlink="<a href='aj.php?id="+pid+"&im="+cur+"&nxt="+nxt+" .ajcontent' data-featherlight='ajax'></a>";

//$(this).href=''aj.php?id='+pid+'&im='+cur+'&nxt='+nxt+' .ajcontent'';
//$(this).attr('')=imlink
//imlink.click();
...
<script type="text/javascript">
    ...
    function loadMoreData(last_id){
      $.ajax(
            {
                url: 'load_moredata.php?last_id=' + last_id,
                type: "get",
                beforeSend: function()
                {
                    $('.ajax-load').show();
...
$(document).on('click', '.post-comment-btn', function() {
 ...
 $(document).on('click', '.post-media-comment-btn', function() {
 
 $(this).parent().next().next().find('.post-media-comment-box').show();
...
$(document).on('click', '.post-media-like-button', function() {
 
	var post_id=$(this).closest('.one-col-post').attr('id');
	var media_id=$(this).attr('id');
	
	$.ajax({
        	url: "post_func.php",
			type: "POST",
			data: { postid: post_id, mediaid: media_id },
    	                cache: false,			
			success: function(data)
		    {
	        
			if($('#'+media_id+'.post-media-like-button').text()=='Like') {
				$('#'+media_id+'.post-media-like-button').text('Unlike');
			}
			else if($('#'+media_id+'.post-media-like-button').text()=='Unlike'){
				$('#'+media_id+'.post-media-like-button').text('Like');
			}
				
			
			
				//alert(parseInt($('#number-likes').html()));
				$('.media-number-likes'+media_id).html(data);
				$('.media-number-likes'+media_id).prev('#paw-likes').toggleClass('paw-like');
...
<script>
 $(document).ready(function() {
 $(document).on('click', '.media-comment-submit', function() {
	...
	//alert(comment_data);
		...
		$.ajax({
			...
			type: "POST",
			data: { 
			postmediaid: post_id , mediacommentid: media_id, mediacommenttext: comment_data
			},
    	    cache: false,			
			success: function(data)
		    {
	          $('#'+media_id+'.post-media-comment-box').hide();
			  $('#'+media_id+'.post-media-comment-btn').parent().next().next().html(data);
			  //$('#'+post_id+'.post-comment-box').parent().append("<div class='user-comments'>"+data+"</div>");
