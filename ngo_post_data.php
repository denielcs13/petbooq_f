...
WHILE ($postres = mysqli_fetch_assoc($disprun)) {
    ...
    $par_name = mysqli_query($conn, "SELECT * FROM ngo_resgitration WHERE ngo_unique_id='$postres[child_post_id]'");
        ...
        <div class="one-col-inn">
            <div class="post-in-c">
                <div class="post-content">
					...
					<?php
    if ($pname_res['bg_image'] == "") {
        echo "<img src='images/fr-pro-big-img.jpg'>";
    } else {
        ?>
                                <img src="<?= $pname_res['bg_image'] ?>" alt="user image" />
                            <?php } ?>
							</span>
		<p class="user-nm"><a href="about3.php?id=<?= $postres['child_post_id'] ?>"><?= $pname_res['ngo_name'] ?></a></p></h2>
                    <h3><?= $postres['title'] ?></h3>
        ...
        for ($i = 0; $i < count($post_images); $i++) {
            ?>
                                        ...
                                        <li class="pb-image-box">
                                            <a href="aj_ngo.php?id=<?= $postres['id'] ?>&ngid=<?= $ngo_id ?>&im=<?= $post_images[$i] ?> .ajcontent" data-featherlight="ajax">
    ...
    if ($postres['vid_count'] > 0) {
        ...
        for ($i = 0; $i < count($post_videos); $i++) {
            ?>
                                        ...
                                        <li class="pb-image-box">
                                            <a href="aj_ngo.php?id=<?= $postres['id'] ?>&ngid=<?= $ngo_id ?>&vid=<?= $post_videos[$i] ?> .ajcontent" data-featherlight="ajax">
                                                <video controls>
        ...
        <?php }
                ...
                <div class="post-act-btn">
                    ...
                    <?php
                     require './dbcon.php';
                     $select ="select * from ngo_like where ngo_id ='$_GET[id]' and user_id = '$user_id'";
