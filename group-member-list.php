...
$search = $_POST["search"];
require './dbcon.php';
$results = "select * from pet_rehome";
$query = mysqli_query($conn, $results);
$columns = mysqli_num_rows($query);
if(isset($_POST['search'])) {
$results = "SELECT pet_name,about_pet,pet_photo,species,status from pet_rehome WHERE pet_name LIKE '%".$search."%' or species LIKE '%".$search."%'";
}
else {
    $results = "SELECT pet_name,about_pet,pet_photo,species,status from pet_rehome";
}
...
$adopt = 1;
require './dbcon.php';
$results = "update pet_rehome set status = '$adopt' where id = '$id'";
