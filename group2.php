...
<?php require_once 'inc/header11.php';  ?>
    <?php
        ...
        $gr_name=$_GET[name];        
        require 'dbcon.php';
        $userqry = mysqli_query($conn, "SELECT * FROM groups WHERE group_id='$_GET[id]' AND user_id_fk='$parent_id'");
        $userinf = mysqli_fetch_assoc($userqry);
        //$display = "SELECT * FROM updates WHERE user_id_fk='$parent_id' AND group_id_fk='$_GET[id]' ORDER BY update_id DESC LIMIT 4";
        //$display = "SELECT * FROM updates WHERE group_id_fk='$_GET[id]' ORDER BY update_id DESC LIMIT 4";
...
?>
<div class="grp-name"><h2><i class="fa fa-paw"></i><br><span class="grp-name-t"><?php echo $display['group_name']?></span></h2></div>
<div class="sidebar">
<ul>
...
<h4>Group Setting</h4>
<ul>
<li><a href="#"><i class="fa fa-user"></i>Manage Group</a></li>
<li><a href="members-list.php?id=<?php echo $group_id;?>&name=<?php echo $gr_name;?>"><i class="fa fa-users"></i>Members</a></li>
<li><a href="#"><i class="fa fa-user"></i>Discussion</a></li>
...
<li><a href="#"><i class="fa fa-image"></i>Photos</a></li>
	...
	$image_tmpname=$_FILES['file']['tmp_name'];
        $target_path = $id ."/Groups/".$gp_name."/profile_pic/" . basename($_FILES['file']['name']);
        $brk_path = $id . "/Groups/".$gp_name."/profile_pic/";
...
<button class="post-join-button" id="<?= $_GET['id'] ?>"><i class="fa fa-sign-in"></i>Join</button>
<button class="post-ntf-button" id=""><i class="fa fa-bell"></i>Notification<span class="ntf-cont">02<span></button>
...
<input type="hidden" value="<?= $_GET[id] ?>" name="group-id">
<input type="hidden" value="<?= $_GET[name] ?>" name="group-name">
</div>
...
$gp_id=$_GET['id'];
//$parent_id = $_SESSION["pet_unique_id"];
$grpqry=mysqli_query($conn,"SELECT U.pet_name, U.pet_unique_id,U.profile_pic
...
session_start();
$parent_id = $_SESSION["pet_unique_id"];
$grpqry=mysqli_query($conn,"SELECT * FROM groups WHERE group_id='$gp_id' and status='1'");
					WHILE($grpres=mysqli_fetch_assoc($grpqry)) {
						?>
