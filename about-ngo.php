...
<div class="main-content user-pro-dtls-page usr-feed-page-nw about-page usr-feed-page groups-page business-dtl-page width-80" id="about-load-ref">
...
<div class="pro-timeline"><?php if($userinf['bg_image']=="") {
...
//$propostqry=mysqli_query($conn,"SELECT * FROM post WHERE child_post_id='$parent_id' ORDER BY id DESC LIMIT 4");
								WHILE($propost=mysqli_fetch_assoc($disprun)) {
...
<a href="#">
	<?php if($userinf['profile_image']=="") {
							echo "<img src='images/fr-pro-big-img.jpg'>";
							}
							else {
							?>
							<img src="<?= $userinf['profile_image'] ?>" alt="user image" />
							<?php } ?>

</a></span><p class="user-nm"><a href="about3.php?id=<?= $userinf['ngo_unique_id'] ?>"><?= $userinf['ngo_name'] ?></a></p></h2>
<h3><?= $propost['title'] ?></h3>
...
<a href="aj_pbox.php?id=<?= $propost['id'] ?>&im=<?= $proimages[$i] ?> .ajcontent" data-featherlight="ajax">
<img src="<?= $proimages[$i] ?>"></a>
											...
											<?php if($propost['vid_count']>0) {
												...
												?>   
												<li class="pb-image-box">
											<a href="aj_pbox.php?id=<?= $propost['id'] ?>&vid=<?= $provid[$i] ?> .ajcontent" data-featherlight="ajax"><video width="230" height="200" controls>
...
<!--postactend-->											
								...
								<?php } ?>
<?php include 'post-share-tst.php'; ?>
...
<script>
		$(document).on('click','#comment-load',function(e) {
			e.preventDefault();
			var postid=$(this).closest('.post-in-c').attr('id');
			$.ajax({
